...
//    ]
//
//  📌 HOW MATCHING WORKS (3 levels, in order):
//
...
//       If overlap ≥ 60% of user's words → it's a match.
//       Example:
//         User: "kya kr rhi ho aajkal"     (5 words)
//         Dataset: "kya kr rhi ho"          (4 words)
//         Overlap: 4 words → 4/5 = 80% → MATCH ✅
//
//    3. PARTIAL MATCH  (loose fallback)
//       Checks if any single word from user's message
//       appears in the dataset entry's user text.
//       Only triggers if the match score is the best so far.
//
...
export function getDatasetReply(userMessage, dataset) {
  ...
  for (const entry of dataset) {
    ...
    // ── Level 2: High word overlap ─────────────────────────
    const entryWords = wordSet(entry.user)
    const overlap = overlapRatio(userWords, entryWords)

    // 60% or more word overlap = strong match
    if (overlap >= 0.6 && overlap > bestScore) {
      bestScore = overlap
      bestMatch = entry
    }

    // ── Level 3: Partial / substring match (loose fallback) ─
    // Only as a tiebreaker if nothing better found yet
    if (overlap < 0.6 && bestScore < 0.3) {
      // Check if the entire user input is a substring of the dataset entry
      // OR dataset entry is a substring of user input
      if (
        normalizedEntry.includes(normalizedInput) ||
        normalizedInput.includes(normalizedEntry)
      ) {
        // Score it low — only used as last resort
        const substringScore = 0.2
